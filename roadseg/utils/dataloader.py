"""Dataloader for Road-Graph Dataset
- Load inputs as,
	- Satellite images.
- Load outputs as, 
	- Target probability of vertex & edges; 
	- Target vector of edges;
	- Graph-tensor segments;
- Copy from: (modified in adaption to py37)
	https://github.com/songtaohe/Sat2Graph/blob/master/model/dataloader.py 
...
class Sat2GraphDataLoader():
	...
	def loadtile(self, ind):
		...
		try:
			...
			sat_img = np.array(Image.open(self.folder + "/region_%d_sat.png" % ind)).astype(np.float)
		except:
			# sat_img = scipy.ndimage.imread(self.folder + "/region_%d_sat.jpg" % ind).astype(np.float)
			sat_img = np.array(Image.open(self.folder + "/region_%d_sat.jpg" % ind)).astype(np.float)
	...
	def preload(self, num = 1024, seg_only = False):
		...
		for i in range(self.preload_tiles):
			...
			try:
				...
				sat_img = np.array(Image.open(self.folder + "/region_%d_sat.png" % ind))
			except:
				# sat_img = scipy.ndimage.imread(self.folder + "/region_%d_sat.jpg" % ind)
				sat_img = np.array(Image.open(self.folder + "/region_%d_sat.jpg" % ind))

			max_v = np.amax(sat_img) + 0.0001 

			# Load the locations (as arry index) of vertexes along with their own neighbors (Format: {vertex: [neighbor_vertexes]})
			neighbors = pickle.load(open(self.folder + "/region_%d_refine_gt_graph.p" % ind, "rb"))
			neighbors = neighbor_to_integer(neighbors)
			...
			gt_seg = np.array(Image.open(self.folder + "/region_%d_gt.png" % ind))
			...
			# Restore a graph-tensor according to the info of `neighbors`
			for loc, n_locs in neighbors.items():
...
class Sat2GraphDataset(Dataset):
    def __init__(self, folder, indrange=[0,10], imgsize=256, preload_tiles=4, max_degree=6, loadseg=False, random_mask=True, testing=False, dataset_image_size=2048, transpose=False):
        self.loader = Sat2GraphDataLoader(folder, indrange, imgsize, preload_tiles, max_degree, loadseg, random_mask, testing, dataset_image_size, transpose)

    def __len__(self):
        return self.loader.preload_tiles

    def __getitem__(self, idx):
        sat_img, tiles_prob, tiles_vector = self.loader.loadtile(idx)

        # Convert to PyTorch tensors
        sat_img = torch.from_numpy(sat_img)
        tiles_prob = torch.from_numpy(tiles_prob)
        tiles_vector = torch.from_numpy(tiles_vector)
...
if __name__ == "__main__":
	...
	osmdataset = "/home/aylive/workspace/Sat2Graph/data/20cities"

	dataloader_train = Sat2GraphDataLoader(osmdataset, imgsize = 352, preload_tiles = 4, testing = False, random_mask=True)
	dataloader_train.preload(num=1024)

	input_sat, gt_prob, gt_vector, gt_seg = dataloader_train.getBatch(2)